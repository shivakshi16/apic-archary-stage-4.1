...
// Write code to detect collision between player arrow and opponent
  ...
  var archerCollison=Matter.SAT.collides(
    playerArrows[i].body,
    computerArcher.body
  )
  var computerCollison=Matter.SAT.collides(
    playerArrows[i].body,
    computer.body
  )
  if(
...
function handleComputerArrowCollision() {
  for(var i=0; i< computerArrows.length; i++){
    ...
    var playerCollison=Matter.SAT.collides(
      computerArrows[i].body,
      player.body
    )
    if(
  ...
  MediaStreamAudioDestinationNode.collided.SAT("I am a")
}
